...
```{r logo, echo=FALSE}
htmltools::img(src = knitr::image_uri("MiDAS_logo.png"), 
               alt = 'logo', 
...
```{r show_imported_data , echo=FALSE, warning=FALSE, include=TRUE}
...
```{r plot_frequencies, echo=TRUE, warning=FALSE, fig.width=7, fig.height=7}

freq_HLA_long <- gather(freq_HLA, population, freq, Freq, "USA NMDP European Caucasian", "USA NMDP Chinese", "USA NMDP African American pop 2", factor_key=TRUE) %>% filter(grepl("^A", allele))

plot_HLAfreq <- ggplot(data=freq_HLA_long, aes(x=allele, y=freq, fill=population)) + geom_bar(stat="identity", position=position_dodge(0.7), width=0.7, colour="black") + coord_flip() + scale_y_continuous(labels = percent)
...
We can know test our HLA data for deviations from Hardy-Weinberg-Equilibrium (HWE) to filter out alleles that strongly deviate from HWE expectations (imputation or genotyping errors,...). Here, let's remove alleles with a HWE P value below 0.05 divided by the number of alleles tested / present in our data (N=447). We can create a filtered MiDAS object right away (`as.MiDAS = TRUE`), as done in this example, or output actual HWE test results.

```{r HWE_filtering, echo=TRUE, warning=FALSE}
HLA <- HWETest(HLA, experiment="hla_alleles", HWE_cutoff = 0.05/447, as.MiDAS = TRUE)
```
...
HLA_results_cond <- runMiDAS(
  ...
  correction = "bonferroni", 
  exponentiate = TRUE)
kableResults(HLA_results_cond, scroll_box_height = "250px")
```
...
```{r HLA_AA_to_df , echo=TRUE, warning=FALSE, include=TRUE}
dat_HLA_AA <- as.data.frame(t(HLA_AA@ExperimentList@listData[["hla_aa"]]@assays@data@listData[[1]])) %>% select(starts_with("DRB1_1"))
```

```{r show_HLA_AA , echo=FALSE, warning=FALSE, include=TRUE}
...
```{r run_HLA_AA_omnibus, echo=TRUE, warning=FALSE}
...
kableResults(HLA_AA_DQB1_9_results, scroll_box_height = "300px")
```
...
kable(HLA_AA_DQB1_9_alleles) %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed")) %>%
...
HLA_het <- prepareMiDAS(hla_calls = dat_HLA, colData = dat_pheno, experiment = c("hla_het","hla_divergence"))
