...
#   Hmisc::label(midas_hla_allele_test[1], self = FALSE) <- "id"
...
#   i <- c(colnames(pheno)[-1], colnames(covar[-1]))
#   Hmisc::label(midas_hla_allele_test[, i], self = FALSE) <-
#     rep("additional_variable", (ncol(pheno) + ncol(covar) - 2))
#   midas_hla_allele_test$term <- 1
#   Hmisc::label(midas_hla_allele_test[, "term"], self = FALSE) <- "placeholder"
#   attributes(midas_hla_allele_test)$call <- list(
#     quote(prepareMiDAS),
#     hla_calls = quote(hla_calls),
#     quote(pheno),
#     quote(covar),
#     analysis_type = "hla_allele",
#     inheritance_model = "additive"
#   )
#   class(midas_hla_allele_test) <- c("midas.data", class(midas_hla_allele_test))
